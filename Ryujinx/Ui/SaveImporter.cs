...
namespace Ryujinx.Ui {
  internal class SaveImporter {
    ...
    public int Import() { return ImportSaves(FsClient, ImportPath); }

    private static int ImportSaves(FileSystemClient fsClient,
                                   string rootSaveDir) {
      ...
      foreach(SaveToImport save in finder.Saves) {
        ...
        if (importResult.IsFailure()) {
          throw new HorizonResultException(importResult,
                                           $"Error importing save {save.Path}");
        }
      }
    ...
    private static Result ImportSave(FileSystemClient fs, SaveToImport save) {
      ...
      Result result =
          fs.CreateSaveData(key.TitleId, key.UserId, key.TitleId, 0, 0, 0);
      if (result.IsFailure())
        return result;
      ...
      try {
        result =
            fs.Register("OldSave".ToU8Span(), new LocalFileSystem(save.Path));
        if (result.IsFailure())
          return result;
        ...
        result =
            fs.MountSaveData("NewSave".ToU8Span(), key.TitleId, key.UserId);
        if (result.IsFailure())
          return result;
    ...
    private class SaveFinder {
      public List<SaveToImport>Saves { get; }
      = new List<SaveToImport>();
      ...
      private void SearchUser(string path, ulong saveDataId, UserId userId) {
        foreach(string subDir in Directory.EnumerateDirectories(path)) {
          if (TryGetUInt64(subDir, out ulong titleId) &&
              TryGetDataPath(subDir, out string dataPath)) {
            SaveDataAttribute attribute = new SaveDataAttribute{
                Type = SaveDataType.Account, UserId = userId,
      ...
      private static bool TryGetDataPath(string path, out string dataPath) {
        ...
        if (Directory.Exists(committedPath) &&
            Directory.EnumerateFileSystemEntries(committedPath).Any()) {
          dataPath = committedPath;
          return true;
        }

        if (Directory.Exists(workingPath) &&
            Directory.EnumerateFileSystemEntries(workingPath).Any()) {
