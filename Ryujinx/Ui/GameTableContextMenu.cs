...
#pragma warning restore IDE0044
    ...
    public GameTableContextMenu(ListStore gameTableStore, TreeIter rowIter,
                                VirtualFileSystem virtualFileSystem)
        : this(new Builder("Ryujinx.Ui.GameTableContextMenu.glade"),
               gameTableStore, rowIter, virtualFileSystem) {}

    private GameTableContextMenu(Builder builder, ListStore gameTableStore,
                                 TreeIter rowIter,
                                 VirtualFileSystem virtualFileSystem)
        : base(builder.GetObject("_contextMenu").Handle) {
      ...
      string ext =
          System.IO.Path
              .GetExtension(_gameTableStore.GetValue(_rowIter, 9).ToString())
              .ToLower();
    ...
    private bool TryFindSaveData(string titleName, string titleIdText,
                                 out ulong saveDataId) {
      saveDataId = default;

      if (!ulong.TryParse(titleIdText, NumberStyles.HexNumber,
      ...
      Result result = _virtualFileSystem.FsClient.FindSaveDataWithFilter(
      ...
      if (ResultFs.TargetNotFound.Includes(result)) {
        ...
        using MessageDialog messageDialog = new MessageDialog(
            null, DialogFlags.Modal, MessageType.Question, ButtonsType.YesNo,
            null){
            Title = "Ryujinx",
            Icon = new Gdk.Pixbuf(Assembly.GetExecutingAssembly(),
        ...
        result = _virtualFileSystem.FsClient.CreateSaveData(
            new TitleId(titleId), new UserId(1, 0), new TitleId(titleId), 0, 0,
            0);
        ...
        result = _virtualFileSystem.FsClient.FindSaveDataWithFilter(
    ...
    private string GetSaveDataDirectory(ulong saveDataId) {
      string saveRootPath = System.IO.Path.Combine(
          _virtualFileSystem.GetNandPath(), $"user/save/{saveDataId:x16}");
      ...
      // If the committed directory exists, that path will be loaded the next
      // time the savedata is mounted
      if (Directory.Exists(committedPath)) {
        return committedPath;
      }

      // If the working directory exists and the committed directory doesn't,
      // the working directory will be loaded the next time the savedata is
      // mounted
    ...
    private void ExtractSection(NcaSectionType ncaSectionType) {
      FileChooserDialog fileChooser = new FileChooserDialog(
          "Choose the folder to extract into", null,
      ...
      if (response == (int) ResponseType.Accept) {
        Thread extractorThread = new Thread(() => {
          ...
          Gtk.Application.Invoke(delegate {
            _dialog = new MessageDialog(null, DialogFlags.DestroyWithParent,
                ...
                Icon = new Gdk.Pixbuf(Assembly.GetExecutingAssembly(),
                                      "Ryujinx.Ui.assets.Icon.png"),
                SecondaryText =
                    $"Extracting {ncaSectionType} section from {System.IO.Path.GetFileName(sourceFile)}...",
                WindowPosition = WindowPosition.Center};

            int dialogResponse = _dialog.Run();
            if (dialogResponse == (int) ResponseType.Cancel ||
                dialogResponse == (int) ResponseType.DeleteEvent) {
              _cancel = true;
              _dialog.Dispose();
            }
          });
          ...
          using(FileStream file = new FileStream(sourceFile, FileMode.Open,
            ...
            if ((System.IO.Path.GetExtension(sourceFile).ToLower() == ".nsp") ||
                (System.IO.Path.GetExtension(sourceFile).ToLower() ==
                 ".pfs0") ||
            ...
            } else if (System.IO.Path.GetExtension(sourceFile).ToLower() ==
                       ".nca") {
            ...
            if (mainNca == null) {
              Logger.PrintError(
                  ...
                  "Extraction failed. The main NCA was not present in the selected file.");
            ...
            int index = Nca.GetSectionIndexFromType(ncaSectionType,
                                                    mainNca.Header.ContentType);

            IFileSystem ncaFileSystem =
                patchNca !=
                null ? mainNca
                    .OpenFileSystemWithPatch(patchNca, index,
                                             IntegrityCheckLevel.ErrorOnInvalid)
                : mainNca.OpenFileSystem(index,
                                         IntegrityCheckLevel.ErrorOnInvalid);
            ...
            fsClient.Register(output.ToU8Span(),
                              new LocalFileSystem(destination));

            (Result? resultCode, bool canceled) =
                CopyDirectory(fsClient, $"{source}:/", $"{output}:/");
            ...
            if (!canceled) {
              if (resultCode.Value.IsFailure()) {
                ...
                Gtk.Application.Invoke(delegate {
                  ...
                  GtkDialog.CreateErrorDialog(
                      "Extraction failed. Read the log file for further information.");
                });
    ...
    private(Result? result, bool canceled)
        CopyDirectory(FileSystemClient fs, string sourcePath, string destPath) {
      Result rc =
          fs.OpenDirectory(out DirectoryHandle sourceHandle,
                           sourcePath.ToU8Span(), OpenDirectoryMode.All);
      if (rc.IsFailure())
        return (rc, false);

      using(sourceHandle) {
        foreach(DirectoryEntryEx entry in fs.EnumerateEntries(
            sourcePath, "*", SearchOptions.Default)) {
          if (_cancel) {
            return (null, true);
          }

          string subSrcPath =
              PathTools.Normalize(PathTools.Combine(sourcePath, entry.Name));
          string subDstPath =
              PathTools.Normalize(PathTools.Combine(destPath, entry.Name));

          if (entry.Type == DirectoryEntryType.Directory) {
            fs.EnsureDirectoryExists(subDstPath);

            (Result? result, bool canceled) =
    ...
    public Result CopyFile(FileSystemClient fs, string sourcePath,
                           string destPath) {
      Result rc = fs.OpenFile(out FileHandle sourceHandle,
                              sourcePath.ToU8Span(), OpenMode.Read);
      if (rc.IsFailure())
        return rc;
    ...
    private void OpenSaveDir_Clicked(object sender, EventArgs args) {
      string titleName =
          _gameTableStore.GetValue(_rowIter, 2).ToString().Split("\n")[0];
      string titleId = _gameTableStore.GetValue(_rowIter, 2)
                           .ToString()
                           .Split("\n")[1]
                           .ToLower();
      ...
      Process.Start(new ProcessStartInfo(){
          FileName = saveDir, UseShellExecute = true, Verb = "open"});
    }
