...
namespace Ryujinx.HLE.HOS.Services.Settings
{
    ...
    class ISystemSettingsServer : IpcService
    {
        ...
        [Command(4)]
        // GetFirmwareVersion2() ->
        // buffer<nn::settings::system::FirmwareVersion, 0x1a, 0x100>
        public ResultCode
            GetFirmwareVersion2(ServiceCtx context)
        {
        ...
        // GetColorSetId() -> i32
        public ResultCode
            GetColorSetId(ServiceCtx context)
        {
        ...
        // GetColorSetId() -> i32
        public ResultCode
            SetColorSetId(ServiceCtx context)
        {
        ...
        [Command(37)]
        // GetSettingsItemValueSize(buffer<nn::settings::SettingsName, 0x19>,
        // buffer<nn::settings::SettingsItemKey, 0x19>) -> u64
        public ResultCode
            GetSettingsItemValueSize(ServiceCtx context)
        {
            ...
            string askedSetting = Encoding.ASCII.GetString(Class).Trim('\0') + "!" +
                                  Encoding.ASCII.GetString(name).Trim('\0');
        ...
        [Command(38)]
        // GetSettingsItemValue(buffer<nn::settings::SettingsName, 0x19, 0x48>,
        // buffer<nn::settings::SettingsItemKey, 0x19, 0x48>) -> (u64,
        // buffer<unknown, 6, 0>)
        public ResultCode
            GetSettingsItemValue(ServiceCtx context)
        {
            long classPos = context.Request.PtrBuff[0].Position;
            long classSize = context.Request.PtrBuff[0].Size;
                ...
                if (nxSetting is string stringValue)
                {
                    if (stringValue.Length + 1 > replySize)
                    {
                        Logger.PrintError(LogClass.ServiceSet,
                                          $"{askedSetting} String value size is too big!");
                    }
                    else
                    {
        ...
        public byte[] GetFirmwareData(Switch device)
        {
            ...
            string contentPath = device.System.ContentManager.GetInstalledContentPath(
                titleId, StorageId.NandSystem, NcaContentType.Data);
            ...
            string firmwareTitlePath =
                device.FileSystem.SwitchPathToSystemPath(contentPath);

            using (IStorage firmwareStorage =
                      new LocalStorage(firmwareTitlePath, FileAccess.Read))
            {
                ...
                IFileSystem firmwareRomFs = firmwareContent.OpenFileSystem(
                    NcaSectionType.Data, device.System.FsIntegrityCheckLevel);

                Result result = firmwareRomFs.OpenFile(
                    out IFile firmwareFile, "/file".ToU8Span(), OpenMode.Read);
                if (result.IsFailure())
                {
                    return null;
                }
