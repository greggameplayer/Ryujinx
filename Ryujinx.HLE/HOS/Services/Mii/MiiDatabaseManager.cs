...
namespace Ryujinx.HLE.HOS.Services.Mii {
  class MiiDatabaseManager {
    ...
    private bool GetAtVirtualIndex(int index, out int realIndex,
    ...
    private Result MountSave() {
      ...
      if (MountCounter == 0) {
        ...
        if (IsTestModeEnabled) {
          ...
          // Nintendo use NS TitleID when creating the production save even on
          // sdb, let's follow that behaviour.
          targetTitleId = NsTitleId;
        }

        U8Span mountName = new U8Span(MountName);

        result = _filesystemClient.MountSystemSaveData(
            mountName, SaveDataSpaceId.System, targetSaveDataId);

        if (result.IsFailure()) {
          if (ResultFs.TargetNotFound.Includes(result)) {
            // TODO: We're currently always specifying the owner ID because FS
            // doesn't have a way of knowing which process called it
            result = _filesystemClient.CreateSystemSaveData(
                targetSaveDataId, new TitleId(targetTitleId), 0x10000, 0x10000,
                SaveDataFlags
                    .KeepAfterResettingSystemSaveDataWithoutUserSaveData);
            if (result.IsFailure())
              return result;
    ...
    public ResultCode DeleteFile() {
      ResultCode result =
          (ResultCode) _filesystemClient.DeleteFile(DatabasePath).Value;
    ...
    public ResultCode LoadFromFile(out bool isBroken) {
      ...
      Result result = _filesystemClient.OpenFile(out FileHandle handle,
                                                 DatabasePath, OpenMode.Read);
    ...
    private Result ForceSaveDatabase() {
      Result result = _filesystemClient.CreateFile(
          DatabasePath, Unsafe.SizeOf<NintendoFigurineDatabase>());

      if (result.IsSuccess() || ResultFs.PathAlreadyExists.Includes(result)) {
        result = _filesystemClient.OpenFile(out FileHandle handle, DatabasePath,
                                            OpenMode.Write);
        ...
        if (result.IsSuccess()) {
          ...
          if (result.IsSuccess()) {
            ...
            if (fileSize != Unsafe.SizeOf<NintendoFigurineDatabase>()) {
              ...
              if (result.IsSuccess()) {
                result = _filesystemClient.CreateFile(
                    DatabasePath, Unsafe.SizeOf<NintendoFigurineDatabase>());

                if (result.IsSuccess()) {
                  result = _filesystemClient.OpenFile(out handle, DatabasePath,
                                                      OpenMode.Write);
                }
              }
            ...
            result = _filesystemClient.WriteFile(
                handle, 0, _database.AsReadOnlySpan(), WriteOption.Flush);
          }
    ...
    public DatabaseSessionMetadata
    CreateSessionMetadata(SpecialMiiKeyCode miiKeyCode) {
      return new DatabaseSessionMetadata(UpdateCounter, miiKeyCode);
    }

    public void SetInterfaceVersion(DatabaseSessionMetadata metadata,
    ...
    public void Get(DatabaseSessionMetadata metadata, int index,
                    out StoreData storeData) {
      if (!metadata.MiiKeyCode.IsEnabledSpecialMii()) {
    ...
    public ResultCode FindIndex(DatabaseSessionMetadata metadata, out int index,
                                CreateId createId) {
      return FindIndex(out index, createId,
                       metadata.MiiKeyCode.IsEnabledSpecialMii());
    }

    public ResultCode FindIndex(out int index, CreateId createId,
                                bool isSpecial) {
    ...
    public ResultCode Move(DatabaseSessionMetadata metadata, int newIndex,
                           CreateId createId) {
      if (!metadata.MiiKeyCode.IsEnabledSpecialMii()) {
    ...
    public ResultCode AddOrReplace(DatabaseSessionMetadata metadata,
                                   StoreData storeData) {
    ...
    public ResultCode Delete(DatabaseSessionMetadata metadata,
                             CreateId createId) {
    ...
    public bool IsFullDatabase() { return _database.IsFull(); }
  }
}
