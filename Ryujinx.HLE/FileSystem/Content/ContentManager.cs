...
namespace Ryujinx.HLE.FileSystem.Content
{
    public class ContentManager
    {
        ...
        private SortedDictionary<(ulong titleId, NcaContentType type), string>
            _contentDictionary;
        ...
        public void LoadEntries(Switch device = null)
        {
            lock (_lock)
            {
                _contentDictionary =
                    new SortedDictionary<(ulong, NcaContentType), string>();
                _locationEntries =
                    new Dictionary<StorageId, LinkedList<LocationEntry>>();
                ...
                foreach (StorageId storageId in Enum.GetValues(typeof(StorageId)))
                {
                    ...
                    try
                    {
                        ...
                        contentDirectory = LocationHelper.GetRealPath(_virtualFileSystem,
                                                                      contentPathString);
                    ...
                    LinkedList<LocationEntry> locationList =
                        new LinkedList<LocationEntry>();
                        ...
                        registeredDirectory))
                    {
                        ...
                        {
                            string ncaName = new DirectoryInfo(directoryPath)
                                                 .Name.Replace(".nca", string.Empty);

                            using (FileStream ncaFile =
                                      File.OpenRead(Directory.GetFiles(directoryPath)[0]))
                            {
                                Nca nca =
                                    new Nca(_virtualFileSystem.KeySet, ncaFile.AsStorage());

                                string switchPath =
                                    contentPathString + ":/" +
                                    ncaFile.Name.Replace(contentDirectory, string.Empty)
                                ...
                                LocationEntry entry =
                                    new LocationEntry(switchPath, 0, (long)nca.Header.TitleId,
                                                      nca.Header.ContentType);

                                AddEntry(entry);

                                _contentDictionary.Add(
                                    (nca.Header.TitleId, nca.Header.ContentType), ncaName);
                            }
                        }
                    }

                    foreach (
                        string filePath in Directory.EnumerateFiles(contentDirectory))
                    {
                        if (Path.GetExtension(filePath) == ".nca")
                        {
                                                                      ...
                                                                      FileAccess.Read))
                            {
                                ...
                                string switchPath =
                                    contentPathString + ":/" +
                                    filePath.Replace(contentDirectory, string.Empty)
                                        .TrimStart(Path.DirectorySeparatorChar);
                    ...
                    if (_locationEntries.ContainsKey(storageId) &&
                        _locationEntries[storageId]?.Count == 0)
                    {
        ...
        public void ClearEntry(long titleId, NcaContentType contentType,
                               StorageId storageId)
        {
        ...
        public bool HasNca(string ncaId, StorageId storageId)
        {
            lock (_lock)
            {
                ...
                {
                    var content =
                        _contentDictionary.FirstOrDefault(x => x.Value == ncaId);
                    long titleId = (long)content.Key.Item1;

                    NcaContentType contentType = content.Key.type;
                    StorageId storage =
                        GetInstalledStorage(titleId, contentType, storageId);
        ...
        public UInt128 GetInstalledNcaId(long titleId, NcaContentType contentType)
        {
            lock (_lock)
            {
                ...
                {
                    return new UInt128(
                        _contentDictionary[((ulong)titleId, contentType)]);
                }
            }
                                             ...
                                             StorageId storageId)
        {
            lock (_lock)
            {
                LocationEntry locationEntry =
                    GetLocation(titleId, contentType, storageId);

                return locationEntry.ContentPath !=
                       null ? LocationHelper
                           .GetStorageId(locationEntry.ContentPath)
                    : StorageId.None;
            }
        }

        public string GetInstalledContentPath(long titleId, StorageId storageId,
                                              NcaContentType contentType)
        {
            lock (_lock)
            {
        ...
        public void RedirectLocation(LocationEntry newEntry, StorageId storageId)
        {
            ...
            {
                LocationEntry locationEntry =
                    GetLocation(newEntry.TitleId, newEntry.ContentType, storageId);
        ...
        private bool VerifyContentType(LocationEntry locationEntry,
                                       NcaContentType contentType)
        {
            if (locationEntry.ContentPath == null)
            {
            ...
            string installedPath =
                _virtualFileSystem.SwitchPathToSystemPath(locationEntry.ContentPath);

            if (!string.IsNullOrWhiteSpace(installedPath))
            {
                if (File.Exists(installedPath))
                {
                    using (FileStream file = new FileStream(installedPath, FileMode.Open,
                                                           FileAccess.Read))
                    {
        ...
        private void RemoveLocationEntry(long titleId, NcaContentType contentType,
                                         StorageId storageId)
        {
            LinkedList<LocationEntry> locationList = null;
            ...
            if (locationList != null)
            {
                LocationEntry entry = locationList.ToList().Find(
        ...
        private LocationEntry GetLocation(long titleId, NcaContentType contentType,
                                          StorageId storageId)
        {
            LinkedList<LocationEntry> locationList = _locationEntries[storageId];

            return locationList.ToList().Find(x => x.TitleId == titleId &&
        ...
        public void InstallFirmware(string firmwareSource)
        {
            string contentPathString =
                LocationHelper.GetContentRoot(StorageId.NandSystem);
            string contentDirectory =
                LocationHelper.GetRealPath(_virtualFileSystem, contentPathString);
                ...
                switch (info.Extension)
                {
                    ...
                    default:
                        throw new InvalidFirmwarePackageException(
                            "Input file is not a valid firmware package");
                }
        ...
        private void FinishInstallation(string temporaryDirectory,
        ...
        private void InstallFromDirectory(string firmwareDirectory,
                                          string temporaryDirectory)
        {
            InstallFromPartition(new LocalFileSystem(firmwareDirectory),
                                 temporaryDirectory);
        }

        private void InstallFromPartition(IFileSystem filesystem,
                                          string temporaryDirectory)
        {
            foreach (var entry in filesystem.EnumerateEntries("/", "*.nca"))
            {
                Nca nca = new Nca(_virtualFileSystem.KeySet,
                                  OpenPossibleFragmentedFile(filesystem, entry.FullPath,
                                                             OpenMode.Read)
                                      .AsStorage());

                SaveNca(nca, entry.Name.Remove(entry.Name.IndexOf('.')),
                        temporaryDirectory);
            }
        }
        ...
        private void InstallFromCart(Xci gameCard, string temporaryDirectory)
        {
            if (gameCard.HasPartition(XciPartitionType.Update))
            {
                XciPartition partition =
                    gameCard.OpenPartition(XciPartitionType.Update);
        ...
        private void InstallFromZip(ZipArchive archive, string temporaryDirectory)
        {
            using (archive)
            {
                ...
                {
                    if (entry.FullName.EndsWith(".nca") ||
                        entry.FullName.EndsWith(".nca/00"))
                    {
                        // Clean up the name and get the NcaId

                        string[] pathComponents =
                            entry.FullName.Replace(".cnmt", "").Split('/');

                        string ncaId = pathComponents[pathComponents.Length - 1];

                        // If this is a fragmented nca, we need to get the previous
                        // element.GetZip
                        if (ncaId.Equals("00"))
                        {
        ...
        private IFile OpenPossibleFragmentedFile(IFileSystem filesystem,
        ...
        public SystemVersion VerifyFirmwarePackage(string firmwarePackage)
        {
            Dictionary<ulong, List<(NcaContentType type, string path)>> updateNcas =
                new Dictionary<ulong, List<(NcaContentType, string)>>();
            ...
            SystemVersion VerifyAndGetVersionZip(ZipArchive archive)
            {
                ...
                foreach (var entry in archive.Entries)
                {
                        ...
                        entry.FullName.EndsWith(".nca/00"))
                    {
                        using (Stream ncaStream = GetZipStream(entry))
                        {
                            ...
                            {
                                updateNcas[nca.Header.TitleId].Add(
                                    (nca.Header.ContentType, entry.FullName));
                            }
                            else
                            {
                                updateNcas.Add(nca.Header.TitleId,
                                               new List<(NcaContentType, string)>());
                                updateNcas[nca.Header.TitleId].Add(
                                    (nca.Header.ContentType, entry.FullName));
                            }
                        }
                ...
                if (updateNcas.ContainsKey(SystemUpdateTitleId))
                {
                    ...
                    string metaPath =
                        ncaEntry.Find(x => x.type == NcaContentType.Meta).path;
                    ...
                    {
                        Nca metaNca =
                            new Nca(_virtualFileSystem.KeySet, ncaStream.AsStorage());

                        IFileSystem fs = metaNca.OpenFileSystem(NcaSectionType.Data,
                                                                integrityCheckLevel);

                        string cnmtPath =
                            fs.EnumerateEntries("/", "*.cnmt").Single().FullPath;

                        if (fs.OpenFile(out IFile metaFile, cnmtPath.ToU8Span(),
                                        OpenMode.Read)
                                .IsSuccess())
                        {
                    ...
                    {
                        string versionEntry = updateNcas[SystemVersionTitleId]
                                                  .Find(x => x.type != NcaContentType.Meta)
                                                  .path;

                        using (Stream ncaStream =
                                  GetZipStream(archive.GetEntry(versionEntry)))
                        {
                            Nca nca =
                                new Nca(_virtualFileSystem.KeySet, ncaStream.AsStorage());

                            var romfs =
                                nca.OpenFileSystem(NcaSectionType.Data, integrityCheckLevel);

                            if (romfs
                                    .OpenFile(out IFile systemVersionFile, "/file".ToU8Span(),
                                              OpenMode.Read)
                                    .IsSuccess())
                            {
                    ...
                    foreach (CnmtContentMetaEntry metaEntry in metaEntries)
                    {
                        if (updateNcas.TryGetValue(metaEntry.TitleId, out ncaEntry))
                        {
                            ...
                            // Nintendo in 9.0.0, removed PPC and only kept the meta nca of
                            // it. This is a perfect valid case, so we should just ignore the
                            // missing content nca and continue.
                            if (contentPath == null)
                            {
                            ...
                            using (Stream metaNcaStream = GetZipStream(metaZipEntry))
                            {
                                ...
                                {
                                    Nca metaNca = new Nca(_virtualFileSystem.KeySet,
                                                          metaNcaStream.AsStorage());
                                            ...
                                            .IsSuccess())
                                    {
                                        ...
                                        if (contentStorage.GetSize(out long size).IsSuccess())
                                        {
                                            ...
                                            if (LibHac.Util.ArraysEqual(
                                                    hash.ToArray(), meta.ContentEntries[0].Hash))
                                            {
                                                updateNcas.Remove(metaEntry.TitleId);
                                            }
                                        }
            ...
            SystemVersion VerifyAndGetVersion(IFileSystem filesystem)
            {
                ...
                foreach (var entry in filesystem.EnumerateEntries("/", "*.nca"))
                {
                    IStorage ncaStorage = OpenPossibleFragmentedFile(
                                              filesystem, entry.FullPath, OpenMode.Read)
                                              .AsStorage();

                    Nca nca = new Nca(_virtualFileSystem.KeySet, ncaStorage);

                    if (nca.Header.TitleId == SystemUpdateTitleId &&
                    ...
                    {
                        updateNcas[nca.Header.TitleId].Add(
                            (nca.Header.ContentType, entry.FullPath));
                    }
                    else
                    {
                        updateNcas.Add(nca.Header.TitleId,
                ...
                foreach (CnmtContentMetaEntry metaEntry in metaEntries)
                {
                    ...
                    {
                        var metaNcaEntry =
                            ncaEntry.Find(x => x.type == NcaContentType.Meta);
                        string contentPath =
                            ncaEntry.Find(x => x.type != NcaContentType.Meta).path;

                        // Nintendo in 9.0.0, removed PPC and only kept the meta nca of it.
                        // This is a perfect valid case, so we should just ignore the
                        // missing content nca and continue.
                        if (contentPath == null)
                        {
                            updateNcas.Remove(metaEntry.TitleId);
                        ...
                        IStorage metaStorage =
                            OpenPossibleFragmentedFile(filesystem, metaNcaEntry.path,
                                                       OpenMode.Read)
                                .AsStorage();
                        IStorage contentStorage =
                            OpenPossibleFragmentedFile(filesystem, contentPath,
                                                       OpenMode.Read)
                                .AsStorage();
                                ...
                                .IsSuccess())
                        {
                            ...
                            if (contentStorage.GetSize(out long size).IsSuccess())
                            {
                                ...
                                if (LibHac.Util.ArraysEqual(hash.ToArray(),
        ...
        public SystemVersion GetCurrentFirmwareVersion()
        {
            ...
            lock (_lock)
            {
                ...
                foreach (var entry in locationEnties)
                {
                    ...
                    {
                        var path =
                            _virtualFileSystem.SwitchPathToSystemPath(entry.ContentPath);

                        using (FileStream fileStream = File.OpenRead(path))
                        {
                            Nca nca =
                                new Nca(_virtualFileSystem.KeySet, fileStream.AsStorage());

                            if (nca.Header.TitleId == SystemVersionTitleId &&
                                nca.Header.ContentType == NcaContentType.Data)
                            {
                                var romfs = nca.OpenFileSystem(NcaSectionType.Data,
                                                               integrityCheckLevel);

                                if (romfs
                                        .OpenFile(out IFile systemVersionFile,
                                                  "/file".ToU8Span(), OpenMode.Read)
                                        .IsSuccess())
                                {
